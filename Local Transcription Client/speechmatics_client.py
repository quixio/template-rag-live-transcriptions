...
def print_transcript(msg):
    ...
    with app.get_producer() as producer:
        json_data = json.dumps({"speaker": SPEAKER, "transcription": msg['metadata']['transcript'], "createdTimestamp": datetime.now().isoformat()})
        producer.produce(
            topic=topic.name,
            key=f"speaker_{SPEAKER}",  # Replace with an appropriate key if needed
            value=json_data,
        )
